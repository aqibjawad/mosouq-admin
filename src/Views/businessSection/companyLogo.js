...
const CompanyLogo = () => {
    ...
    const submit = async (event) => {
        ...
        if (image) {
            formData.append("business_com_image", image);
        }

        try {
            const res = await POST("business-company/add-business-company", formData);
            if (!res.error) {
                toast("Added Done");
                fetchData();
            } else {
        ...
        <div>
            <Col sm={12} className="mt-3">
                <Breadcrumb>
                    ...
                    <Breadcrumb.Item active> Business Companies </Breadcrumb.Item>
                </Breadcrumb>
            </Col>
                                ...
                                <tbody>
                                    {companies.map((company) => (
                                        ...
                                        >
                                            <td>{company.link}</td>
                                            <td>
                                                <img src={`${company.business_com_image}`} style={{ width: "50px", height: "50px" }} />
